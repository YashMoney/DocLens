...
def process_pdf_queries_optimized(pdf_url: str, questions: List[str], groq_api_key: str) -> List[str]:
    ...
    try:
        ...
        extended_prompt = """You are an expert AI assistant specializing in intelligent document analysis and query retrieval for insurance, legal, HR, and compliance domains. You have access to a comprehensive vector database containing processed PDF chunks from various policy documents, contracts, and compliance materials.
...
*Output Format: Present findings clearly with proper source citations, highlighting key information, conditions, and any limitations that apply to the user's specific query Give ouptut in plain text in a single line, highlight the key information if you think we have to extend more than 1 line, please do it only when you think it is necessary.
...
A: 2 years waiting period."""
        ...
        for query in questions:
            ...
            relevant_chunks = retriever.retrieve(query, top_k=10)  # Reduced to 2 for faster processing

            # Combine chunks for context
            context = "\n\n".join([chunk for chunk, score in relevant_chunks])

            # Create focused prompt for concise answers
            prompt = f"""{extended_prompt}, Check for the following examples: {examples}
